...
def calculate_5_week_avg_change(df):
    df['5_week_change'] = df['pct_change'].rolling(window=5).mean()
    return df


def analyze_legacy_df(legacy_df, currency_pairs):
    combined_results = []
    ...
    for base_currency, quote_currency in currency_pairs:
        ...
        merged_df['pair'] = f'{currency_to_symbol[base_currency]}/{currency_to_symbol[quote_currency]}'
        ...
        if quote_currency == "US Dollar":
            ...
            merged_df['pair_net_position'] = -merged_df['quote_net_position']
        else:
            merged_df['pair_long'] = merged_df['base_long']
            merged_df['pair_short'] = merged_df['base_short']
            merged_df['pair_net_position'] = merged_df['base_net_position']
        ...
        merged_df = calculate_5_week_avg_change(merged_df)
...
def filter_and_analyze_legacy_data(final_data):
    ...
    important_headers = [
        'date',
        'pair',
        # Noncommercial Positions-Long (All) for the base currency
        'base_long',
        # Noncommercial Positions-Short (All) for the base currency
        'base_short',
        # Net Noncommercial Positions for the base currency
        'base_net_position',
        # Noncommercial Positions-Long (All) for the quote currency
        'quote_long',
        # Noncommercial Positions-Short (All) for the quote currency
        'quote_short',
        # Net Noncommercial Positions for the quote currency
        'quote_net_position',
    ...
    analyzed_df = analyze_legacy_df(legacy_df, all_currency_pairs).fillna(0)
